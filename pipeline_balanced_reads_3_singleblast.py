...
HYDROGEN_SCRIPT = "/oceanstor/scratch/tllseedorf/e1103389/hydrogen_consumer_pipeline/hydrogen_consumer_pipeline_reads3_4eval_kraken_singleblast.py"
...
BASE_RESULT_DIR  = Path("/oceanstor/scratch/tllseedorf/e1103389/metagenomics_disease/reads3/hydrogen_results")
BASE_TMP_DIR     = Path("/oceanstor/scratch/tllseedorf/e1103389/metagenomics_disease/reads3/tmp/hydrogen_pipe")
...
def run_command(cmd, dry_run=False, capture_output=False):
    print(f"[CMD] {cmd}")
    if dry_run:
        return ""
    if capture_output:
        ...
        #capture the output and put it in a file 
        if ret.returncode != 0:
            #raise the output as well
            raise subprocess.CalledProcessError(ret.returncode, cmd, output=ret.stdout, stderr=ret.stderr)
