...
def create_diamond_database(fasta_file, tmp_folder,diamond_path,train_percent=100):
    ...
    return name
def batch_diamond(joined_records, diamond_exe_path, database_fasta, type, tmp_dir, min_percent_identity=90, min_qcov=0.9, resume=False,paired=False,train_percent=100, thresholds_file=None):
    if thresholds_file is None:
        thresholds_file=r'/oceanstor/scratch/tllseedorf/e1103389/hydrogen_consumer_pipeline/marker_thresholds.csv'
    ...
    #if thresholds file is not none, get the min percent identity and min scov from the file
    print("THRESHOLDS FILE:",thresholds_file)
    if thresholds_file is not None:
        with open(thresholds_file, 'r') as f:
            ...
            for row in reader:
                print("ROW:",row,type)
                if row[0] == type:
                    min_percent_identity = float(row[1])
                    min_qcov = float(row[2])
                    print("using thresholds from file",type,min_percent_identity,min_qcov)
                    break
...
def batch_run_hydrogenase_script(ijoined_records,hydrogenase_script_path,tmp_dir,resume=False,
                                 diamond_exe_path=default_diamond_exe_path,hydb_fasta_file=None,paired=False,train_percent=100, min_pident=50, min_qcov=0.5):
    ...
    if os.path.exists(output_filepath) and not resume:
        try:
            ...
            print("Error removing output directory:", e)
    ...
    #the last argument is the diamond exe path
        command='python '+hydrogenase_script_path+' '+ijoined_records+' '+output_filepath+' '+diamond_exe_path+" --min_pid "+str(min_pident)+" --min_qcov "+str(min_qcov )+" --train "+str(train_percent)
        print("COMMAND:",command)
        os.system(command)
...
def pipeline(reads_file,args):
    ...
    print(len(list(reads)),"number of reads")
    #exit()
    ...
    if "nitrogenase" in open(output_filepath).read():
        print("nitrogenase already in output file, skipping")
        ...
        if hyd:
            ...
            for hyd_grp in hist:
                ...
                for i in range(hist[hyd_grp]):
                    ...
                    print("added",random_rec,"1:N"+str(rand_start))
                    #make a second read
...
def evaluation_script_ind_hydr(root_folder, hyd_grp, marker_fasta,negative_fasta,
                                 sim=True,repeats=3, max_reads=100,percent_junk=0.9,min_hyd_pidents=[80],min_hyd_qcovs=[0.8],HPC=False,res_file=None,train_test_split=0.8):
    num_reads=max_reads 
    num_neg_reads=int(percent_junk*(1/(1-percent_junk))*num_reads)
    '''example of hyd_grp [FeFe]_Group_A1'''
    for repeat in range(repeats):
        ''' extract reads from the marker and negative fasta files, and run the diamond script'''
        #read the marker fasta file
        #if marker fasta file is hyddb, extract the hydrogenase group records
        if "HydDB" in marker_fasta:
            pool=[]
            #open the hyddb fasta file and extract the hyd groups
            with open(marker_fasta, 'r') as f:
                records = list(SeqIO.parse(f, 'fasta'))
                for record in records:
                    if hyd_grp in record.description:
                        pool.append(record)
        else:
            pool=[]
            with open(marker_fasta, 'r') as f:
                ...
                for record in records:
                    pool.append(record)
        #reverse translate the seq
        #and sample reads from it
          #sample a random read
        pos_reads=[]
        for i in range(num_reads):
            random_rec=random.choice(pool)
            random_recname=random_rec.id
            #sample a random read from the record
            
            recseq=reverse_translate(str(random_rec.seq))
            rand_start=random.randint(0,max(0,len(recseq)-100))
            read=recseq[rand_start:rand_start+100]
            #makea  new SeqIO record
            record=SeqIO.SeqRecord(Seq(read), id=random_recname+"_1:N"+str(rand_start))
            #pos_reads.append(">"+random_rec+"1:N"+str(rand_start)+"\n"+str(read))
            pos_reads.append(record)
            print("added",random_rec,"1:N"+str(rand_start))
            #make a second read
            rand_start2=random.randint(max(0,rand_start),max(1,len(recseq)-100))
            read= recseq[rand_start2:rand_start2+100]
            #reverse complement the read
            read=reverse_complement(read)
            #makea  new SeqIO record
            record=SeqIO.SeqRecord(Seq(read), id=random_recname+"_2:N"+str(rand_start))
            pos_reads.append(record)
            #pos_reads.append(">"+random_recname+"2:N"+str(rand_start)+"\n"+str(read))
        neg_reads=[]
        neg_recs= list(SeqIO.parse(negative_fasta, 'fasta'))
        for i in range(num_neg_reads):
                #add reads from the negative fasta file
                rand_neg=random.choice(neg_recs)
                recseq=reverse_translate(str(rand_neg.seq))
                rand_start=random.randint(0,max(len(recseq)-100,0))
                read=recseq[rand_start:rand_start+100]
                recordid=record.id.split("_-_")[0]
                #makea  new SeqIO record
                record=SeqIO.SeqRecord(Seq(read), id=recordid+"_1_N"+str(rand_start))
                neg_reads.append(record)
                #make a second read
                rend=len(recseq)-100
                rand_start2=random.randint(max(rand_start-50,0),max(len(recseq)-100,0))
                read= recseq[rand_start2:rand_start2+100]
                #reverse complement the read
                read=reverse_complement(read)
                #makea  new SeqIO record
                neg_reads.append(SeqIO.SeqRecord(Seq(read), id=recordid+"_2_N"+str(rand_start)))
        #write the reads to a file

        #run the diamond script on pos_reads and neg_reads
        hyd_script_path=r"C:\Users\abel\Documents\hydrogenases\hyDB\Diamond_blast_hyDB_reads.py"
        if HPC:
            hyd_script_path=r'/oceanstor/scratch/tllseedorf/e1103389/hyDB/Diamond_blast_hyDB_reads.py'
        #root_folder=r"C:\Users\abel\Documents\hydrogenases\marker_sens_spec\test_reads"
        #if HPC:
        #    root_folder=r'/oceanstor/scratch/tllseedorf/e1103389/hydrogen_consumer_pipeline/test_reads'
        print("ROOT FOLDER",root_folder)
        tmp_dir=os.path.join(root_folder,"tmp_{}".format(hyd_grp))
        #if tmp dir exist, remove it
        if os.path.exists(tmp_dir):
            shutil.rmtree(tmp_dir)
        os.makedirs(tmp_dir,exist_ok=True)
        #write the reads to a file
        reads_file=os.path.join(tmp_dir,"pos_reads.fasta")
        #write the reads to a file
        with open(reads_file, 'w') as f:
            SeqIO.write(pos_reads, f, "fasta")
        #write the reads to a file
        reads_file_neg=os.path.join(tmp_dir,"neg_reads.fasta")
        #write the reads to a file
        with open(reads_file_neg, 'w') as f:
            SeqIO.write(neg_reads, f, "fasta")


        diamond_exe_path=r"D:\diamond-windows\diamond.exe"
        if HPC:
            diamond_exe_path=r'/oceanstor/scratch/tllseedorf/e1103389/DIAMOND/diamond'
        #if the diamond_exe_path does not exist, use the default diamond_exe_path
        #sftp://abel@hpc.tll.org.sg/tllhome/abel/hydrogen_consumer_pipeline/DIAMOND/diamond
        hyddb_fasta_file=r"C:\Users\abel\Documents\hydrogenases\hyDB\HydDB_reformated.fasta"
        if HPC:
            hyddb_fasta_file=r'/oceanstor/scratch/tllseedorf/e1103389/hyDB/HydDB_reformated.fasta'
        #if the hyddb_fasta_file does not exist, use the default hyddb_fasta_file
        new_tag=hyd_grp.replace("[","").replace("]","")
        res={}
        for min_pident_hyd in min_hyd_pidents:
            for min_qcov_hyd in min_hyd_qcovs:
                if "Fe" in hyd_grp:
                    ...
                    hist1,total=batch_run_hydrogenase_script(reads_file, hyd_script_path, tmp_dir, resume=False, diamond_exe_path=diamond_exe_path, hydb_fasta_file=hyddb_fasta_file,
                                                                paired=True,min_pident=min_pident_hyd,min_qcov=min_qcov_hyd,train_percent=train_test_split*100)
                    
                    abund=hist1[new_tag]
                    sens=abund/num_reads
                    hist2,total=batch_run_hydrogenase_script(reads_file_neg, hyd_script_path, tmp_dir, resume=False, diamond_exe_path=diamond_exe_path, hydb_fasta_file=hyddb_fasta_file,
                                                                paired=True,min_pident=min_pident_hyd,min_qcov=min_qcov_hyd)
                    neg_abund=hist2[new_tag]
                    neg_sens=neg_abund/num_neg_reads
                    prec=abund/(abund+neg_abund+1e-6)
                    accuracy=(abund+(num_neg_reads-neg_abund))/(num_reads+num_neg_reads)
                    res[(min_pident_hyd,min_qcov_hyd)]=[sens,prec,accuracy]
                else:
                    hist1,total=batch_diamond(reads_file, diamond_exe_path, marker_fasta, new_tag, tmp_dir, paired=True, train_percent=train_test_split*100,
                                                min_percent_identity=min_pident_hyd, min_qcov=min_qcov_hyd, resume=False)
                                               
                    abund=hist1
                    sens=abund/num_reads
                    hist2,total=batch_diamond(reads_file_neg, diamond_exe_path, marker_fasta, new_tag, tmp_dir, paired=True,
                                                min_percent_identity=min_pident_hyd, min_qcov=min_qcov_hyd, resume=False)
                    neg_abund=hist2
                    neg_sens=neg_abund/num_neg_reads
                    prec=abund/(abund+neg_abund+1e-6)
        ...
        if res_file is None:
            res_dir=os.path.join(root_folder,"res")
            #make the res dir if it does not exist
            if not os.path.exists(res_dir):
                os.makedirs(res_dir,exist_ok=True)
            res_file=os.path.join(res_dir,"{}_{}_res.txt")
        with open(res_file, 'a') as f:
            #f.write("hyd_grp,min_pident,min_qcov,repeat,sensitivity,precision,accuracy\n")
            for key in res:
                f.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(hyd_grp,key[0],key[1],repeat,res[key][0],res[key][1],res[key][2]))
...
def eval_all_hyds(root_folder, marker_fasta_dir, negative_dir,  max_reads=100, percent_junk=0.9, min_hyd_pidents=[80],min_hyd_qcovs=[0.8],HPC=False, res_file=None):
    #get all the fasta files in the marker_fasta_dir
    marker_fasta_files=[os.path.join(marker_fasta_dir,f) for f in os.listdir(marker_fasta_dir) if f.endswith('.fasta')]
    #get all the fasta files in the negative_dir
    negative_fasta_files=[os.path.join(negative_dir,f) for f in os.listdir(negative_dir) if f.endswith('.fasta')]
    #for each marker fasta file, run the evaluation script
    res_file=os.path.join(root_folder,"res","hyd_res.txt")
    #make the res file
    if not os.path.exists(os.path.dirname(res_file)):
        os.makedirs(os.path.dirname(res_file),exist_ok=True)
    with open(res_file, 'w') as f:
        f.write("hyd_grp,min_pident,min_qcov,repeat,sensitivity,precision,accuracy\n")
    for marker_fasta_file in marker_fasta_files:
        try:
            #skip file if contains "negative"
            if "train" in marker_fasta_file:
                continue

            negative_file=marker_fasta_file.replace(".fasta","_negative.fasta")
            print("marker_fasta_file",marker_fasta_file)
            hyd_grp=os.path.basename(marker_fasta_file).split(".")[0]
            evaluation_script_ind_hydr(root_folder, hyd_grp, marker_fasta_file,negative_file,max_reads=max_reads, 
                                    percent_junk=percent_junk, min_hyd_pidents=min_hyd_pidents,min_hyd_qcovs=min_hyd_qcovs,HPC=HPC,res_file=res_file)

        except Exception as e:
            continue
            print("Error in file",marker_fasta_file)

def eval_all_markers(root_folder, marker_fasta_dir, negative_dir,  max_reads=100, percent_junk=0.9, min_hyd_pidents=[80],min_hyd_qcovs=[0.8],HPC=False, res_file=None):
    #get all the fasta files in the marker_fasta_dir
    marker_fasta_files=[os.path.join(marker_fasta_dir,f) for f in os.listdir(marker_fasta_dir) if f.endswith('.fasta')]
    #get all the fasta files in the negative_dir
    negative_fasta_files=[os.path.join(negative_dir,f) for f in os.listdir(negative_dir) if f.endswith('.fasta')]
    #for each marker fasta file, run the evaluation script
    res_file=os.path.join(root_folder,"res","makrer_res.txt")
    #make the res file
    if not os.path.exists(os.path.dirname(res_file)):
        os.makedirs(os.path.dirname(res_file),exist_ok=True)
    with open(res_file, 'w') as f:
    ...
    for marker_fasta_file in marker_fasta_files:
        try:
            #skip file if contains "negative"
            if "negative" in marker_fasta_file:
                continue
            if "train" in marker_fasta_file:
                continue
            #negative_file=marker_fasta_file.replace(".fasta","_neg.fasta")
            #get the basename of the file
            negative_file=os.path.join(negative_dir,os.path.basename(marker_fasta_file).replace(".fasta","_neg.fasta"))
            print("marker_fasta_file",marker_fasta_file)
            #if neg file no exists, skip the file
            if not os.path.exists(negative_file):
                print("negative file does not exist",negative_file)
                continue
            hyd_grp=os.path.basename(marker_fasta_file).split(".")[0]
            evaluation_script_ind_hydr(root_folder, hyd_grp, marker_fasta_file,negative_file,max_reads=max_reads, 
                                    percent_junk=percent_junk, min_hyd_pidents=min_hyd_pidents,min_hyd_qcovs=min_hyd_qcovs,HPC=HPC,res_file=res_file)
...
def main():
    ...
    args.add_argument('--hydb_fasta_file', help='Enter the path to the hydb database fasta file', default='/oceanstor/scratch/tllseedorf/e1103389/hyDB/HydDB_reformated.fasta')
    #add the hydrogenase script path
    args.add_argument('--hydrogenase_script_path', help='Enter the path to the hydrogenase script', 
                      default='/oceanstor/scratch/tllseedorf/e1103389/hydrogenases/hyDB/Diamond_blast_hyDB_reads2.py')
    ...
    for file in os.listdir(args.input_folder):
        ...
        if ext in["fasta","fa","fna","fastq","fq"]:
            ...
            #if reads file already exists in the tmp folder, skip
            if os.path.exists(os.path.join(args.tmp_folder,new_name)):
                print("skipping",reads_file,"because it already exists in the tmp folder")
                continue
...
def tes3():
    acsb_file=r"C:\Users\abel\acsB.fasta"
    reads_file=r"C:\Users\abel\acsB_reads.fasta"
    #reverse translate the seq
    recs=list(SeqIO.parse(acsb_file, 'fasta'))
    rt=[]
    n_reads=100
    for i in range(n_reads):
        #pick a random record
        rec=random.choice(recs)
        recseq=reverse_translate(str(rec.seq))
        rand_start=random.randint(0,max(0,len(recseq)-100))
        read=recseq[rand_start:rand_start+100]
        #makea  new SeqIO record
        record=SeqIO.SeqRecord(Seq(read), id=rec.id+"_1:N"+str(rand_start))
        rt.append(record)
    #make a new file
    print("num reads",len(rt))
    with open(reads_file, 'w') as f:
        SeqIO.write(rt, f, "fasta")
    diamond_exe=r"C:\Users\abel\Downloads\diamond-windows1\diamond.exe"
    tmp_dir=r"C:\Users\abel\acsB_tmp"
    os.makedirs(tmp_dir,exist_ok=True)
    #run diamond
    #make the db
    db_file=r"C:\Users\abel\acsB_db.dmnd"
    if not os.path.exists(db_file):
        os.system("{} makedb --in {} -d {}".format(diamond_exe,acsb_file,db_file))
    #run diamond
    cmd=f"{diamond_exe} blastx -d {db_file} -q {reads_file} -o {tmp_dir}/diamond_output.tsv --outfmt 6 --max-target-seqs 1 --threads 4 --sensitive --evalue 0.001"
    os.system(cmd)
    #parse the reads
    df=pd.read_csv(os.path.join(tmp_dir,"diamond_output.tsv"), sep="\t", header=None)
    #count the number of individual hits
    df.columns=["query","subject","pident","length","mismatch","gapopen","qstart","qend","sstart","send","evalue","bitscore"]
    print(len(df["query"].unique()),"unique hits")
    #count the number of reads
    print(len(rt),"total reads")
...
if __name__=="__main__":
    ...
    #tes3()
    #exit()
    #hydb_file=r"C:\Users\abel\Documents\hydrogenases\hyDB\HydDB_reformated.fasta"
    #neg_dir=r"C:\Users\abel\Documents\hydrogenases\negative_datasets"
    #marker_dir=r"C:\Users\abel\Documents\hydrogenases\curated_marker\final_curated_marker"
    #eval_all_markers(r"C:\Users\abel\Documents\hydrogenases\marker_sens_spec\test_reads",
    #                 marker_dir, neg_dir, max_reads=100,percent_junk=0.9,min_hyd_pidents=[80],min_hyd_qcovs=[0.8],HPC=False,res_file=None)
    
   
    hydb_file=r"C:\Users\abel\Documents\hydrogenases\hyDB\HydDB_reformated.fasta"
    neg_dir=r"/oceanstor/scratch/tllseedorf/e1103389/marker_sens_spec"
    marker_dir=r"/oceanstor/scratch/tllseedorf/e1103389/marker_sens_spec"
    eval_all_hyds(r"/oceanstor/scratch/tllseedorf/e1103389/marker_sens_spec/test_reads2",
                     marker_dir, neg_dir, max_reads=100,percent_junk=0.9,min_hyd_pidents=[20,50,80,90 ],min_hyd_qcovs=[0.2,0.5,0.7,0.9],HPC=True,res_file=None)

    #exit()
    neg_dir=r"/oceanstor/scratch/tllseedorf/e1103389/marker_sens_spec/negative_datasets"
    marker_dir=r"/oceanstor/scratch/tllseedorf/e1103389/marker_sens_spec/final_curated_marker"
    eval_all_markers(r"/oceanstor/scratch/tllseedorf/e1103389/marker_sens_spec/test_reads",
                     marker_dir, neg_dir, max_reads=100,percent_junk=0.9,min_hyd_pidents=[20,50,80,90 ],min_hyd_qcovs=[0.2,0.5,0.7,0.9],HPC=True,res_file=None)
    
  
    #evaluation_script_ind_hydr("[NiFe]_Group_1a", hydb_file,neg_file)
    #evaluation_script(sim=True,train_test_split=1,repeat=1,percent_junk=0.9,hyd=True,marker=False,junk=True)
    #evaluation_script_HPC(sim=True,train_test_split=1,repeats=1,percent_junk=0.999,hyd=True,marker=True,junk=True,
    #                      min_pidents=[5,20,50,70,90],min_qcovs=[0.5,0.7,0.9,1])