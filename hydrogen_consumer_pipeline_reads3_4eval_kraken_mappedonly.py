...
# === Helpers for running Kraken on only mapped reads ===
import gzip, tempfile
from typing import Iterable, Set, Tuple, List
from Bio import SeqIO

def collect_mapped_ids_from_csv(csv_path: str, id_col: str = "query", sep: str = ",") -> set:
    r"""Read a mapping CSV and return the set of read IDs (column `query` by default)."""
    ids = set()
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        try:
            ...
            for candidate in ("read", "read_id", "qseqid", "Query", "Read"):
                if candidate in header:
                    idx = header.index(candidate)
                    break
            else:
                raise RuntimeError(f"ID column '{id_col}' not found in {csv_path}. Header={header}")
        for line in f:
            if not line.strip():
                continue
            parts = line.rstrip("\\n").split(sep)
            if idx < len(parts):
                rid = parts[idx]
                if rid:
                    ids.add(_normalize_read_id(rid))
    return ids

def _normalize_read_id(rid: str) -> str:
    return rid.split()[0]

def _make_id_variants(rid: str) -> Tuple[str, str, str]:
    raw = rid
    core = rid
    if raw.endswith("/1") or raw.endswith("/2"):
        core = raw[:-2]
    return raw, core, core + "/1"

def _detect_bio_format(path: str) -> str:
    p = path.lower()
    if p.endswith((".fa", ".fna", ".fasta", ".fa.gz", ".fna.gz", ".fasta.gz")):
        return "fasta"
    elif p.endswith((".fq", ".fastq", ".fq.gz", ".fastq.gz")):
        return "fastq"
    with (gzip.open(path, "rt") if p.endswith(".gz") else open(path, "rt")) as f:
        c = f.read(1)
    return "fastq" if c == "@" else "fasta"

def write_subset_reads(input_reads: str, keep_ids: set, out_path: str) -> str:
    r"""Write only records whose ID is in keep_ids; supports FASTA/FASTQ and gz."""
    if not keep_ids:
        raise ValueError("No mapped read IDs to keep; nothing to subset.")
    fmt = _detect_bio_format(input_reads)
    opener = gzip.open if input_reads.endswith(".gz") else open
    n_in = n_out = 0
    with opener(input_reads, "rb") as handle_in, open(out_path, "wb") as handle_out:
        for rec in SeqIO.parse(handle_in, fmt):
            n_in += 1
            rid = _normalize_read_id(rec.id)
            raw, core, core1 = _make_id_variants(rid)
            if (raw in keep_ids) or (core in keep_ids) or (core1 in keep_ids):
                n_out += 1
                SeqIO.write(rec, handle_out, fmt)
    print(f"[SUBSET] Selected {n_out} of {n_in} records into {out_path}")
    if n_out == 0:
        raise RuntimeError("Subset contains 0 reads; ID mismatch? Check mapping CSV IDs vs FASTA/FASTQ headers.")
    return out_path
...
def pipeline(args):
    ...
    # [MOD] Skip early Kraken; will run after mapping
    ...
    read_to_tax = 
    # === Run Kraken2 only on mapped reads ===
    hyd_csv = os.path.join(blast_dir, "Filtered", "read_to_subject.csv")
    hyd_ids = collect_mapped_ids_from_csv(hyd_csv, id_col="query", sep=",")

    marker_ids = set()
    try:
        # If marker_map_list exists; else collect from combined marker output dir
        for _m in marker_map_list if 'marker_map_list' in locals() else []:
            if isinstance(_m, dict) and 'read_to_subject_csv' in _m:
                marker_ids |= collect_mapped_ids_from_csv(_m['read_to_subject_csv'], id_col="query", sep=",")
    except Exception as _e:
        print("[WARN] Could not enumerate per-marker CSVs from marker_map_list:", _e)

    # Fallback: scan marker combined/filtered directory for CSVs
    marker_filtered_dir = os.path.join(combined_marker_dir if 'combined_marker_dir' in locals() else out_dir_abs, "markers", "Filtered")
    if os.path.isdir(marker_filtered_dir):
        for fn in os.listdir(marker_filtered_dir):
            if fn.endswith("read_to_subject.csv"):
                marker_ids |= collect_mapped_ids_from_csv(os.path.join(marker_filtered_dir, fn), id_col="query", sep=",")

    mapped_ids = hyd_ids | marker_ids
    print(f"[MAPPED] HydDB IDs: {len(hyd_ids)} | Marker IDs: {len(marker_ids)} | Union: {len(mapped_ids)}")

    if len(mapped_ids) == 0:
        print("[MAPPED] No mapped reads; skipping Kraken classification.")
        read_to_tax = {}
    else:
        subset_ext = "fa" if _detect_bio_format(reads1) == "fasta" else "fq"
        subset_reads = os.path.join(tmp_dir_abs if 'tmp_dir_abs' in locals() else out_dir_abs, f"mapped_reads.subset.{subset_ext}")
        write_subset_reads(reads1, mapped_ids, subset_reads)

        kraken_out_tsv = os.path.join(out_dir_abs, "kraken", "classified.tsv")
        run_kraken2(
            reads1=subset_reads,
            kraken_db=args.kraken_db,
            output_path=kraken_out_tsv,
            kraken_exe=args.kraken_exe,
            threads=args.kraken_threads,
        )
        read_to_tax = parse_kraken_output(kraken_out_tsv)
    parse_kraken_output(kraken_out)
