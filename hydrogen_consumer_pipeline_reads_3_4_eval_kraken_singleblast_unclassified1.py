...
def load_markers_map(markers_map_tsv: str):
    ...
    pairs = []
    with open(markers_map_tsv, 'r', newline='') as f:
        for line in f:
    ...
    if not hydb_fasta:
        for cand in DEFAULT_HYDB_FASTA_CANDIDATES:
            if os.path.exists(cand):
                hydb_fasta = cand
                break
    print("Using HydDB FASTA:", hydb_fasta)
...
def build_arg_parser():
    ...
    p.add_argument("--hydb_fasta", default='/oceanstor/scratch/tllseedorf/e1103389/hyDB/HydDB_reformated2.fasta', help="HydDB reformatted FASTA (protein). If omitted, tries common locations.")
    ...
    # MARKERS inputs
    p.add_argument("--markers_map_tsv", required=False,default='/oceanstor/scratch/tllseedorf/e1103389/hydrogen_consumer_pipeline/markers.tsv',
