...
hydrogen_classes_activity_file= r"C:\Users\abel\Documents\hydrogenases\hyDB\hydrogenase_classes_activity2t.csv"
taxon_output=r"C:\Users\abel\Documents\mouse_gut\hydrogen_consumer_pipeline\out.BAT.bin2classification_name.txt"
DEBUG_DIR='/tllhome/abel/hydrogen_consumer_pipeline/test/test_genomes/batches'
import csv
import re
import os
    ...
    with open(combined_hyd_tsv,'r') as f:
        ...
        for row in reader:
            ...
            if row[0]!='':
                ...
                #pdb.set_trace()
    return data
def deduplicate_evidence_rows(combine_hyd_tsv,out_file):
    entry_dict={}
    with open(combine_hyd_tsv,'r') as f:
        current_genome=None
        reader=csv.reader(f,delimiter='\t')
        #skip the header
        next(reader)
        for row in reader:
            if len(row) == 0:
                continue
            #if row[0] is not empty, set the current genome
            if row[0]!='':
                current_genome=row[0]
                #if current genome is not in the entry dict, add it
                if current_genome not in entry_dict:
                    entry_dict[current_genome]=set()
                #add the row to the entry dict
                #entry_dict[current_genome].add(tuple(row))
            #if row[0] is empty, add the row to the entry dict
            else:
                entry_dict[current_genome].add(tuple(row))
    #write the entry dict to the file
    with open(out_file,'w') as f:
        writer=csv.writer(f,delimiter='\t')
        header=['filename','hydrogen_consumer','sulphate_reducer','methanogen','acetogen','consuming hydrogenase','bifurcating_or_bidirectional_hydrogenase','dsr_matches','mcr_matches','acetoscan_matches','hyDB_matches']
        writer.writerow(header)
        for genome in entry_dict:
            #write the row for each genome
            genome_row=[genome,'','','','','','','','','','']
            writer.writerow(genome_row)
            for row in entry_dict[genome]:
                #if row is not only consisting of empty strings, write the row
...
MGBC000011.fasta	taxid assigned	based on 4388/4437 ORFs	1;131567;2;1783270;68336;976;200643;171549;815;816	1.00;0.99;0.99;0.87;0.87;0.87;0.87;0.87;0.74;0.73	root (no rank): 1.00	cellular organisms (no rank): 0.99	Bacteria (superkingdom): 0.99	FCB group (clade): 0.87	Bacteroidetes/Chlorobi group (clade): 0.87	Bacteroidetes (phylum): 0.87	Bacteroidia (class): 0.87	Bacteroidales (order): 0.87	Bacteroidaceae (family): 0.74	Bacteroides (genus): 0.73
    ...
    print("combined_hyd_tsv",combined_hyd_tsv)
    parsed_dict=parse_combined_hyd_file(combined_hyd_tsv,hydrogen_classes_activity_file=hydrogen_classes_activity_file)

    with open('/tllhome/abel/hydrogen_consumer_pipeline/test/test_genomes/batches/parsed_dict.txt', 'w') as f:
    
        print("Parsed dict\n",str(parsed_dict), file=f)
    #open the out file
    with open(out_file,'w') as f:
        ...
        for genome in parsed_dict:
            ...
            for hydrogenase in data_dict['hyDB_matches']:
                ...
                a_match=True
            #if there is no match, write a row with NA for the protein match, hyd_direction, hyd_localization, hyd_oxygen_tolerance
            if not a_match:
                hyd_direction='NA'
                hyd_localization='NA'
                hyd_oxygen_tolerance='NA'
                datadict_row, taxon_rows=make_row_data(genome_name=genome,datadict=data_dict,taxon_dict=taxon_dict)
                #add the genome name, datadict row, taxon rows, NA, NA, NA, NA
                writer.writerow([genome]+datadict_row+taxon_rows+['NA','NA','NA','NA']) 
    return out_file
...
def tes2():
    hydrogen_consumer_pipeline_output_folder=r"C:\Users\abel\Documents\hydrogen_consumers\hydrogen_consumer_pipeline_test\dummy_output"
    taxon_output=r"C:\Users\abel\Desktop\scratch\cft_test\combined_classification_file.txt"
    hydrogen_classes_activity_file= r"C:\Users\abel\Desktop\scratch\cft_test\hydrogenase_classes_activity2t.csv"
    hydrogen_consumer_pipeline_output_folder=r"C:\Users\abel\Desktop\scratch\cft_test"
    out_file=r"C:\Users\abel\Desktop\scratch\cft_test\all_output_debug.csv"
    out_file=combine_batches(hydrogen_consumer_pipeline_output_folder,
        r"C:\Users\abel\Desktop\scratch\cft_test\all_output_debug.csv")
    #deduplicate the rows
    out_file=deduplicate_evidence_rows(out_file,r"C:\Users\abel\Desktop\scratch\cft_test\all_output_debug1.csv")
    make_feature_table(combined_hyd_tsv=out_file,out_file=r"C:\Users\abel\Desktop\scratch\cft_test\feature_table_test.tsv",
                       hydrogen_classes_activity_file=hydrogen_classes_activity_file,taxon_output=taxon_output)
    exit()
def arg_parser():
