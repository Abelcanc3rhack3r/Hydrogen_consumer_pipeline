...
import pickle
def parse_results(positive_file,test_file, test_results, negative_file, negative_results, min_pid, min_scov):
    #parse the train file and get the length of the sequences
    train_lengths={}
    #if pickle file exists, read it
    positive_pickle_file=positive_file.replace(".fasta", "_lengths.pickle")
    if os.path.exists(positive_pickle_file):
        with open(positive_pickle_file, "rb") as handle:
    ...
    test_count=0
    #write a pickle file to store the train lengths
    with open(positive_pickle_file, "wb") as handle:
        pickle.dump(train_lengths, handle)
    ...
    #get the sensitivity,if denominator is 0, return 0
...
def parse_results_batched(train_file,test_file, test_results, negative_file, negative_results, min_pids, min_scovs,gene_name,filtered_folder,train_percent,trial):
    ...
    for min_pid in min_pids:
        for min_scov in min_scovs:
            ...
            true_positives+=pseudocount
            false_positives+=pseudocount
            test_count+=pseudocount
            negative_count+=pseudocount
...
def diamond_blast(train_file, test_file, negative_recs_file,diamond_exe,cwd_folder,tag=""):
    ...
    #run the diamond blast,dont run if size of test file is 0
...
def training_split(positive_file, train_percent,cwd_folder,tag=""):
    ...
    train_size=int(len(positive_records)*train_percent)+1
...
def run (positive_file, negative_file, diamond_exe, cwd_folder, train_percent, min_pid, min_scov,trial):
    ...
    #tag=str(train_percent)+"_"+str(min_pid)+"_"+str(min_scov)+"_"+str(trial)
    tag=str(train_percent)+"_"+str(trial)
    ...
    if os.path.exists(os.path.join(cwd_folder, tag+"_test_results.tsv")):
        ...
        #return test_results, negative_results
        #parse_results_batched(train_file,test_file, test_results, negative_file, negative_results, min_pids, min_scovs,gene_name,filtered_folder,train_percent,trial):
        row=parse_results_batched(positive_file, test_file, test_results, negative_file, negative_results, min_pid, min_scov)
        return row
    else:
        test_results, negative_results=diamond_blast(train_file, test_file, negative_file, diamond_exe, cwd_folder, tag)
    #parse the results
    row=parse_results(positive_file, test_file, test_results, negative_file, negative_results, min_pid, min_scov)
    return row

def run_hyd (positive_file, negative_file, diamond_exe, cwd_folder, train_percent, min_pids, min_scovs,trial,group_name):
    #split the training file
    #tag=str(train_percent)+"_"+str(min_pid)+"_"+str(min_scov)+"_"+str(trial)
    tag=str(train_percent)+"_"+str(trial)
    train_file, test_file=training_split(positive_file, train_percent,cwd_folder,tag)
    filtered_folder=os.path.join(cwd_folder, "filtered")
    #run the diamond blast
    #if tagged file exists
    if os.path.exists(os.path.join(cwd_folder, tag+"_test_results.tsv")):
        test_results=os.path.join(cwd_folder, tag+"_test_results.tsv")
        ...
        #return test_results, negative_results
        #parse_results_batched(train_file,test_file, test_results, negative_file, negative_results, min_pids, min_scovs,gene_name,filtered_folder,train_percent,trial):
        rows=parse_results_batched(positive_file, test_file, test_results, negative_file, negative_results, min_pids, min_scovs,group_name, filtered_folder,train_percent,trial)
        return rows
    else:
        test_results, negative_results=diamond_blast(train_file, test_file, negative_file, diamond_exe, cwd_folder, tag)
    #parse the results
...
def run_hydrogenase(pos_file, neg_file, diamond_exe, cwd_folder,train_percents, min_pids, min_scovs,trial_count,marker_name,hyd_group):
    ...
    for train_percent in train_percents:
                for trial in range(trial_count):
                        ...
                        try:
                            rows1=run_hyd(positive_file, negative_file, diamond_exe, hyd_folder, train_percent, min_pids, min_scovs,trial,hyd_group)
...
def run_all_hydrogenases( cwd_folder):
    train_percents=[0.8,0.2]
    min_pids=[0,10,20,40,60,80,90]
    min_scovs=[0,0.3,0.5,0.7,0.9]
    trial_count=3
...
out_folder=r'C:\Users\abel\Documents\hydrogenases\marker_sens_spec'
...
cwd_folder=r"C:\Users\abel\Documents\hydrogenases\marker_sens_spec"
