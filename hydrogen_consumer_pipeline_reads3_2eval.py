...
def evaluation_script_HPC(sim=True,repeats=3, max_reads=100,percent_junk=0.9,train_test_split=0.8,junk=False,marker=True,hyd=True,
                          min_pidents=[80],min_qcovs=[0.8]):
    for repeat in range(repeats):
        '''sqame as the evaluation script, but use the HPC paths'''
        ...
        if sim:
            default_hyddb_fasta_file=r'/tllhome/abel/hydrogen_consumer_pipeline/hyDB/HydDB_reformated.fasta'
            hyddb_fasta_file=r"/oceanstor/home/e1103389/hydrogenases/hyDB/HydDB_reformated.fasta"
            #if hyddb_fasta_file does not exist, use the default hyddb_fasta_file
            if not os.path.exists(hyddb_fasta_file):
                hyddb_fasta_file=default_hyddb_fasta_file
            hyd_groups=defaultdict(list)
            ...
            if hyd:
                ...
                for hyd_grp in hist:
                    ...
                    for i in range(hist[hyd_grp]):
                        #sample a random read
                        random_rec=random.choice(hyd_groups[hyd_grp])
                        #if len of seq is 0, continue
                        while len(rec_dict[random_rec].seq)==0:
                            random_rec=random.choice(hyd_groups[hyd_grp])
                        #sample a random read from the record
                        record=rec_dict[random_rec]
                        gene_stats[record.id][0]=len(record.seq)*3
            ...
            if marker:
                marker_files={"mcrA":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/mcrA.fasta',
                            "acsB":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/acsB.fasta',
                                "dsrA":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/dsrA.fasta',
                                "aprA":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/aprA.fasta',
                                "asrA":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/asrA.fasta',
                                "dmsA":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/dmsA.fasta',
                                "napA":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/napA.fasta',
                                "narG":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/narG.fasta',
                                "nrfA":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/nrfA.fasta',
                                "frdA":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/frdA.fasta',
                                "cydA":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/cydA.fasta',
                                "fhl":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/fhl.fasta',
                                "pfor":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/pfor.fasta',
                                "nitrogenase":'/oceanstor/home/e1103389/hydrogenases/hyDB/final_curated_marker/nitrogenase.fasta'}
            ...
            if junk:
                #junk_genomes_dir=r'C:\Users\abel\Documents\mouse_gut\mouse_gut_catalogue\mgnify_,mouse_catalogue\accepted_genomes'
                #default_junk_genomes_dir=r'/tllhome/abel/mgnify_mouse_catalogue/hq_genomes'
                junk_folders=[("non_consumer",r"/oceanstor/home/e1103389/non_hydrogen_consumer"),
                            ("non_producer",r"/oceanstor/home/e1103389/non_hydrogen_producer")]
                #if junk genome drive not exist, use  /tllhome/abel/mgnify_mouse_catalogue/hq_genomes
                #if not os.path.exists(junk_genomes_dir):
                #    junk_genomes_dir=r'/tllhome/abel/mgnify_mouse_catalogue/hq_genomes'
                junk_genome_files=[]
            ...
            for gene in gene_stats:
                #rkpm = 10^9 * read_mapped/(gene_length*total_reads)
                print(f"hyd_grp_rkpms[{gene_stats[gene][2]}] += 10^9 * ({gene_stats[gene][1]}/({gene_stats[gene][0]}*{len(all_reads)}))")
                hyd_grp_rkpms[gene_stats[gene][2]]+=10**9*(gene_stats[gene][1]/(gene_stats[gene][0]*len(all_reads)))
                #write the rkpm formula to a file
                #with open(r"C:\Users\abel\Documents\mouse_gut\hydrogen_consumer_pipeline\hydrogen_consumer_pipeline_HPC\hydrogen_consumer_pipeline\test_reads\rkpm_formula_gold.txt", 'a') as f:
                #    f.write(f"rkpm{gene}=10^9*{gene_stats[gene][1]}/({gene_stats[gene][0]}*{len(all_reads)} = {10**9*(gene_stats[gene][1]/(gene_stats[gene][0]*len(all_reads)))})\n")
        ...
        #run the diamond blast
        hyd_script_path=r"/oceanstor/scratch/tllseedorf/e1103389/hydrogenases/hyDB/Diamond_blast_hyDB_reads.py"
        #if the hyd_script_path does not exist, use the default hyd_script_path
        if not os.path.exists(hyd_script_path):
            hyd_script_path=r"/tllhome/abel/hydrogen_consumer_pipeline/hyDB/Diamond_blast_hyDB_reads.py"
        tmp_dir=os.path.join(root_folder,"tmp")
        ...
        if not os.path.exists(hyddb_fasta_file):
            hyddb_fasta_file=r"/tllhome/abel/hydrogen_consumer_pipeline/hyDB/HydDB_reformated.fasta"
        if hyd:
            for min_pident_hyd in min_pidents:
                for min_qcov_hyd in min_qcovs:
        ...
        if (marker):
            ...
            for min_pident_hyd in min_pidents:
                for min_qcov_hyd in min_qcovs:
                    for marker in marker_files:
                        count1,rkpm1=batch_diamond(reads_file,diamond_exe_path,marker_files[marker],marker,tmp_dir,paired=True,train_percent=train_test_split*100,min_percent_identity=min_pident_hyd,min_qcov=min_qcov_hyd)
                        #get the rmsd of total and hyd_group_rkpms
                        ...
                        #save the devs to a file
                        #with open(fr"C:\Users\abel\Documents\mouse_gut\hydrogen_consumer_pipeline\hydrogen_consumer_pipeline_HPC\hydrogen_consumer_pipeline\test_reads\marker_devs_{percent_junk}_{len(all_reads)}_{repeat}.txt", 'w') as f:
                        with open (os.path.join(root_folder,f"marker_devs_{percent_junk}_{len(all_reads)}_{repeat}_{min_pident_hyd}_{min_qcov_hyd}.csv"), 'w') as f:
                            #write the header
                            f.write("marker,gold_count,observed_count,gold_rkpm,observed_rkpm,dev,dev_rkpm\n")
                            for key in devs:
                                f.write(key+","+",".join([str(x) for x in devs[key]])+"\n")
    #combine all the devs into a single file
    rows=[["min_pident","min_qcov","gene","gold_count","observed_count","gold_rkpm","observed_rkpm","dev","dev_rkpm"]]
    for repeat in range(repeats):
        for min_pident_hyd in min_pidents:
            for min_qcov_hyd in min_qcovs:
                #if file exists
                if os.path.exists(os.path.join(root_folder,f"_{percent_junk}_{len(all_reads)}_{repeat}_{min_pident_hyd}_{min_qcov_hyd}_devs.csv")):
                    with open(os.path.join(root_folder,f"_{percent_junk}_{len(all_reads)}_{repeat}_{min_pident_hyd}_{min_qcov_hyd}_devs.csv"), 'r') as f:
                        next(f)
                        for line in f:
                            rows.append([str(min_pident_hyd),str(min_qcov_hyd)]+line.strip().split(","))
                            print("row:",rows[-1])
                #if file exists
                if os.path.exists(os.path.join(root_folder,f"marker_devs_{percent_junk}_{len(all_reads)}_{repeat}_{min_pident_hyd}_{min_qcov_hyd}.csv")):
                    with open(os.path.join(root_folder,f"marker_devs_{percent_junk}_{len(all_reads)}_{repeat}_{min_pident_hyd}_{min_qcov_hyd}.csv"), 'r') as f:
                        next(f)
                        for line in f:
                            rows.append([str(min_pident_hyd),str(min_qcov_hyd)]+line.strip().split(","))
                            print("row:",rows[-1])
    
    with open(os.path.join(root_folder,f"all_devs_{percent_junk}_{len(all_reads)}.csv"), 'w') as f:
        for row in rows:
            f.write(",".join(row)+"\n")
...
if __name__=="__main__":
    ...
    evaluation_script_HPC(sim=True,train_test_split=1,repeats=1,percent_junk=0.999,hyd=True,marker=True,junk=True,
                          min_pidents=[5,10,30,50,70,90],
                          min_qcovs=[0.1, 0.3, 0.5,0.7,0.9,1])